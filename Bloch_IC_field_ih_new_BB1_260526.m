...
N10=N0/2; N20=N0/2; N30=N0; N40=N0/2;
tau10=T/2; tau20=T/2; tau30=T; tau40=T/2;
beta10=tau10; beta20=tau20; beta30=tau30; beta40=tau40;
phi_bb1=acos(-0.25);
phi1=0; phi2=phi_bb1; phi3=3*phi_bb1; phi4=phi_bb1;

eta0_vec=linspace(0.9,1,300);
phi0_vec=linspace(0,2*pi,300);
[Eta0,Phi0]=meshgrid(eta0_vec,phi0_vec);
eta0_vec=Eta0(:);
phi0_vec=Phi0(:);
E00=1;
E0_vec=([0.8:0.005:0.9].*E00).';
delta=0;
...
for d=1:length(E0_vec)
    ...
    N3=round(N30.*sqrt((E0.^2+delta.^2)./(E00.^2)));
    N4=round(N40.*sqrt((E0.^2+delta.^2)./(E00.^2)));
    beta1=beta10.*sqrt((E0.^2+delta.^2)./(E00.^2));
    beta2=beta20.*sqrt((E0.^2+delta.^2)./(E00.^2));
    beta3=beta30.*sqrt((E0.^2+delta.^2)./(E00.^2));
    beta4=beta40.*sqrt((E0.^2+delta.^2)./(E00.^2));
    N=N1+N2+N3+N4;
    
    nx1=sin(theta).*cos(phi1); ny1=sin(theta).*sin(phi1); nz1=cos(theta);
    K1=[0,-nz1,ny1;nz1,0,-nx1;-ny1,nx1,0];
    nx2=sin(theta).*cos(phi2); ny2=sin(theta).*sin(phi2); nz2=cos(theta);
    K2=[0,-nz2,ny2;nz2,0,-nx2;-ny2,nx2,0];
    nx3=sin(theta).*cos(phi3); ny3=sin(theta).*sin(phi3); nz3=cos(theta);
    K3=[0,-nz3,ny3;nz3,0,-nx3;-ny3,nx3,0];
    nx4=sin(theta).*cos(phi4); ny4=sin(theta).*sin(phi4); nz4=cos(theta);
    K4=[0,-nz4,ny4;nz4,0,-nx4;-ny4,nx4,0];
    ...
    x_seg1=r(1,:,N1); y_seg1=r(2,:,N1); z_seg1=r(3,:,N1);
    ...
    x_seg2=r(1,:,N1+N2); y_seg2=r(2,:,N1+N2); z_seg2=r(3,:,N1+N2);
    phi_seg2=[phi_seg2,atan2(y_seg2,x_seg2)];
    eta_seg2=[eta_seg2,z_seg2];
    
    for f=(N1+N2):(N1+N2+N3)-1
        r(:,:,f+1)=U3*r(:,:,f);
    end
    x_seg3=r(1,:,N1+N2+N3); y_seg3=r(2,:,N1+N2+N3); z_seg3=r(3,:,N1+N2+N3);
    ...
    x_seg4=r(1,:,N); y_seg4=r(2,:,N); z_seg4=r(3,:,N);
    phi_seg4=[phi_seg4,atan2(y_seg4,x_seg4)];
    eta_seg4=[eta_seg4,z_seg4];
...
K_seg2=boundary(phi_seg2.',eta_seg2.',1);
...
K_seg4=boundary(phi_seg4.',eta_seg4.',1);
Area_seg4=polyarea(phi_seg4(K_seg4),eta_seg4(K_seg4));

R40=Area_seg4./Area_initial;
R30=Area_seg3./Area_initial;
R20=Area_seg2./Area_initial;
R10=Area_seg1./Area_initial;

figure; set(gcf,'color','w'); 
plot(phi_initial,eta_initial,'.k',phi_seg1,eta_seg1,'.r',phi_seg2,eta_seg2,'.g',phi_seg3,eta_seg3,'.b',phi_seg4,eta_seg4,'.c');
grid on; axis([0 2*pi -1 1])
xlabel('$\phi$','interpreter','latex','fontsize',20);
ylabel('$\eta$','interpreter','latex','fontsize',20);
hold on;
plot(phi_initial,eta_initial,'.k',phi_seg1(K_seg1),eta_seg1(K_seg1),'--k',phi_seg2(K_seg2),eta_seg2(K_seg2),'--k',phi_seg3(K_seg3),eta_seg3(K_seg3),'--k',phi_seg4(K_seg4),eta_seg4(K_seg4),'--k');
legend('$0$','$1$','$2$','$3$','$4$','interpreter','latex','fontsize',14);
...
hold on;
x_seg4=sqrt(1-eta_seg4.^2).*cos(phi_seg4);
y_seg4=sqrt(1-eta_seg4.^2).*sin(phi_seg4);
z_seg4=eta_seg4;
plot3(x_seg4,y_seg4,z_seg4,'.c');

%%
hold on;
P_3D=reshape(phi_seg4,sqrt(length(phi0_vec)),sqrt(length(phi0_vec)),length(E0_vec));
E_3D=reshape(eta_seg4,sqrt(length(phi0_vec)),sqrt(length(phi0_vec)),length(E0_vec));
set(gcf,'color','w'); box on; grid on; 
for k=1:length(E0_vec)
    p_3d=P_3D(:,:,k); p_3d=p_3d(:);
    e_3d=E_3D(:,:,k); e_3d=e_3d(:);
    scatter3(p_3d,e_3d,E0_vec(k).*ones(length(p_3d),1),20,E0_vec(k).*ones(length(p_3d),1),'filled'); hold on;
end
alpha 0.5;
xlabel('$\phi$','interpreter','latex','fontsize',20);
ylabel('$\eta$','interpreter','latex','fontsize',20);
zlabel('$\Omega_{1}$','interpreter','latex','fontsize',20);
colorbar;

%%
Etaf_tensor=reshape(eta_seg4,length(Phi0),length(Eta0),length(E0_vec));
Phif_tensor=reshape(phi_seg4,length(Phi0),length(Eta0),length(E0_vec));
...
xlim([1 50]);
xlabel('$\mathcal{G}_{40}$','interpreter','latex','fontsize',20);
ylabel('$N(\mathcal{G}_{40})$','interpreter','latex','fontsize',20);
...
[Eta0,Phi0]=meshgrid(eta0_vec,phi0_vec,E0_vec);
h=histogram2(Phi0,Eta0,'facecolor','k'); xlim([0 2*pi]); ylim([-1 1]);  shading interp;
set(h,'EdgeColor','none'); 

Q_phi=reshape(phi_seg1,length(phi0_vec),length(eta0_vec),length(E0_vec));
Q_eta=reshape(eta_seg1,length(phi0_vec),length(eta0_vec),length(E0_vec));
h=histogram2(Q_phi,Q_eta,'facecolor','r'); xlim([0 2*pi]); ylim([-1 1]); shading interp;
set(h,'EdgeColor','none');

Q_phi=reshape(phi_seg2,length(phi0_vec),length(eta0_vec),length(E0_vec));
Q_eta=reshape(eta_seg2,length(phi0_vec),length(eta0_vec),length(E0_vec));
h=histogram2(Q_phi,Q_eta,'facecolor','g'); xlim([0 2*pi]); ylim([-1 1]); shading interp;
set(h,'EdgeColor','none');

Q_phi=reshape(phi_seg3,length(phi0_vec),length(eta0_vec),length(E0_vec));
Q_eta=reshape(eta_seg3,length(phi0_vec),length(eta0_vec),length(E0_vec));
h=histogram2(Q_phi,Q_eta,'facecolor','b'); xlim([0 2*pi]); ylim([-1 1]); shading interp;
set(h,'EdgeColor','none');

Q_phi=reshape(phi_seg4,length(phi0_vec),length(eta0_vec),length(E0_vec));
Q_eta=reshape(eta_seg4,length(phi0_vec),length(eta0_vec),length(E0_vec));
h=histogram2(Q_phi,Q_eta,'facecolor','c'); xlim([0 2*pi]); ylim([-1 1]); shading interp;
set(h,'EdgeColor','none');
...
%%
all_phi=[mod(phi_initial,2*pi); mod(phi_seg1,2*pi); mod(phi_seg2,2*pi); mod(phi_seg3,2*pi); mod(phi_seg4,2*pi)];
all_eta=[eta_initial; eta_seg1; eta_seg2; eta_seg3; eta_seg4];
...
uncert_phi_seg4=sqrt(mean(phi_seg4.^2)-(mean(phi_seg4)).^2);
uncert_eta_seg4=sqrt(mean(eta_seg4.^2)-(mean(eta_seg4)).^2);

uncert_phi_vec=[uncert_phi_initial,uncert_phi_seg1,uncert_phi_seg2,uncert_phi_seg3,uncert_phi_seg4];
uncert_eta_vec=[uncert_eta_initial,uncert_eta_seg1,uncert_eta_seg2,uncert_eta_seg3,uncert_eta_seg4];
...
uncert_prod_seg4=uncert_phi_seg4.*uncert_eta_seg4;

uncert_prod_vec=[uncert_prod_initial,uncert_prod_seg1,uncert_prod_seg2,uncert_prod_seg3,uncert_prod_seg4];

figure; set(gcf,'color','w'); box on;
hold on;
plot([0,1,2,3,4],uncert_phi_vec,'--ok','MarkerFaceColor',[0, 0.4470, 0.7410]);
plot([0,1,2,3,4],uncert_eta_vec,'--ok','MarkerFaceColor',[0.8500, 0.3250, 0.0980]);
plot([0,1,2,3,4],uncert_prod_vec,'--ok','MarkerFaceColor',[0.4940, 0.1840, 0.5560]);
