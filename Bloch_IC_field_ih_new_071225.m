...
%%
hold on;
P_3D=reshape(phi_seg3,sqrt(length(phi0_vec)),sqrt(length(phi0_vec)),length(E0_vec));
E_3D=reshape(eta_seg3,sqrt(length(phi0_vec)),sqrt(length(phi0_vec)),length(E0_vec));
...
Etai_tensor=reshape(eta_seg2,length(Phi0),length(Eta0),length(E0_vec));
Phii_tensor=reshape(phi_seg2,length(Phi0),length(Eta0),length(E0_vec));
...
nexttile; nbins=500;
set(gcf,'color','w');
histogram(G_tensor(:),nbins,'FaceColor','g'); 
xlim([1 10]);
xlabel('$\mathcal{G}_{32}$','interpreter','latex','fontsize',20);
ylabel('$N(\mathcal{G}_{32})$','interpreter','latex','fontsize',20);
